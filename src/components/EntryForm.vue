    ...
    <template v-slot:default>
        <div class="row row-height">
          <div class="col-height col-md-5">
            <label for="template">{{ labels.template_label }}</label>
            <div class="input-group has-validation" :class="{'has-error': v$.template.$error}">
              <InputMask ref="template" v-model="localData.template" id="template" picture="(50)X" :disabled="disabledKeyField"/> 
              <label class="required">*</label>
            </div>
            <span v-if="v$.template.$error" class="has-error">{{ v$.template.$errors[0].$message }}</span>
          </div>
        </div>
        <div class="row row-height">
          <div class="col-height col-md-5">
            <label for="templatetype">{{ labels.templatetype_label }}</label>
            <div class="input-group has-validation" :class="{'has-error': v$.templatetype.$error}">
              <InputMask ref="templatetype" v-model="localData.templatetype" id="templatetype" picture="(50)X" :disabled="disabledKeyField"/> 
              <label class="required">*</label>
            </div>
            <span v-if="v$.templatetype.$error" class="has-error">{{ v$.templatetype.$errors[0].$message }}</span>
          </div>
        </div>
        <div class="row row-height">
          <div class="col-height col-md-8">
            <label for="subjecttitle">{{ labels.subjecttitle_label }}</label>
            <input ref="subjecttitle" type="text" v-model="localData.subjecttitle" id="subjecttitle" class="form-control input-md" maxlength="100" />
          </div>
        </div>
        <div class="row row-height">
          <div class="col-height col-md-12">
            <label>{{ labels.contents_label }}</label>
            <textarea ref="contents" v-model="localData.contents" id="contents" class="form-control input-md" rows="20" cols="50" />
          </div>
        </div>
    </template>
...
import $ from "jquery";
import { DEFAULT_CONTENT_TYPE, getApiUrl, disableControls, getBaseUrl, getImgUrl }  from '@willsofts/will-app';
...
export default {
  ...
  setup(props) {
    ...
    const validateRules = computed(() => { 
      return {
        template: { required: requiredMessage() },
        templatetype: { required: requiredMessage() },
      } 
    });
  ...
  methods: {
    ...
    startInsertRecord() {
      this.resetRecord();
      this.showDialog(() => { this.$refs.template.focus(); });
    },
    startSaveRecord() {
      confirmSave(() => {
    ...
    startDeleteRecord(dataKeys) {
      confirmDelete([Object.values(dataKeys).join("-")],() => {
        ...
        $.ajax({
          ...
          success: (data) => {
            ...
            successbox(() => {
              ...
              this.resetRecord();
              setTimeout(() => { this.$refs.template.focus(); },100);
      ...
      $.ajax({
        ...
        success: (data) => {
          ...
          if(data.body.dataset) {
            ...
            this.showDialog(() => { this.$refs.subjecttitle.focus(); });
    ...
    initEditor(ttemplatetag) {
      ...
      window.createEditor("#contents",ttemplatetag,getBaseUrl(),getImgUrl());
    },
  }
};
</script>
